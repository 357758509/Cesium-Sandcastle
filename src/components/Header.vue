...
const useDropDownMenu = () => {
  ...
  const initTemplate = (content) => {
    // console.log(data)
    let data = templateData.value[2]
    // let data = templateList[2]
    // let coent = `<template><h1>{{ msg }}</h1></template>
		...
		// 									</style>  
		// 												`
    data.code.VUE.content= content
    // console.log('3434',content,data.content)
    // checkLayout(data)
    nextTick(() => {
      checkLayout(data)
      store.commit('setCode', JSON.parse(JSON.stringify(data.code)))
  ...
  watch(()=>store.state.common.CesiumDomItem,(newVal)=>{
    let data = JSON.parse(newVal)
    console.log("监听列表选项是否发生了变化",data, data.id)
    if(data.id){
      console.log("监听列表选项是否发生了变化2",data)
      initTemplate(data.content)
    }
  },{
    deep: true,
    immediate: true,
